...
final class Shortcodes {
    ...
    public function mint(): string {
        ...
        ob_start(); ?>
        <section class="gr-mint"><p class="gr-eyebrow">INITIALIZATION PROTOCOL</p><h2>AN IDENTITY.<br>NOT YET YOURS.</h2><div class="gr-initialization-closed"><strong>[ INITIALIZATION OFFLINE ]</strong><span>DEPLOYMENT STATUS</span><p>PUBLIC MINT NOT DEPLOYED</p></div><dl class="gr-data-list"><div><dt>PROJECT PHASE</dt><dd>PRE-MINT ARCHIVE</dd></div><div><dt>WALLET CONNECTION</dt><dd>DISABLED</dd></div><div><dt>PLANNED NETWORK</dt><dd>SOLANA</dd></div><div><dt>ARCHIVE RECORDS</dt><dd>3,333</dd></div></dl><h3>PLANNED MINT PHASES</h3><dl class="gr-data-list"><?php foreach (Config::launch_plan()['phases'] ?? [] as $phase => $terms): ?><div><dt><?php echo esc_html(strtoupper($phase)); ?></dt><dd><?php echo esc_html(number_format((float)$terms['price_sol'], 2)); ?> SOL · <?php echo (int)$terms['max_per_wallet']; ?> / WALLET</dd></div><?php endforeach; ?><div><dt>SCHEDULE</dt><dd>NOT ANNOUNCED</dd></div><div><dt>SHARED INVENTORY</dt><dd>3,332 · IDS 0002–3333</dd></div></dl><p class="gr-muted">Independent phase limits: 5 early + 5 public, up to 10 per wallet. Early access requires the finalized allowlist. Both phases use the same inventory. GHOST//0001 is an existing canary and is excluded. These are planned terms; public mint deployment and on-chain enforcement testing remain pending. Buyers pay the mint price plus network fees and asset rent.</p><p class="gr-muted">No wallet connection is required to explore the archive. No payment is required to browse the site. Nothing on this page connects to a wallet, requests a signature, or requests a transaction.</p></section>
        <?php return ob_get_clean();
    }
